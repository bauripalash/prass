...
impl<'pax> Parser<'pax> {
    ...
    fn parse_infix_expr(&mut self, left: Rc<ast::Expr>) -> Rc<ast::Expr> {
        match self.curtok.ttype {
            TokenType::Plus
            | TokenType::Minus
            | TokenType::Mul
    ...
    fn parse_number(&mut self) -> Rc<ast::Expr> {
        ...
        if curtok_lit.split('.').count() == 1 {
            ...
            if let Ok(num) = v {
                ...
                Rc::new(ast::Expr::ErrExpr(ParserError::new(
